...
- 수강 인원이 최대 정원에 도달하면 자동으로 모집 마감 처리되며, 취소가 발생하여 자리가 생기면 자동으로 모집 중으로 복귀 (단, 강의 시작 3일 전이라면 복귀하지 않음)
- 강의 시작 3일 전이 되면 자동으로 모집 마감 처리되어 수강 신청 불가
...
**결제 정책**
- 결제는 결제 대기(PENDING) 상태인 본인 수강 신청 건만 가능
- 이미 결제가 완료되었거나 취소된 건은 결제 불가
- 대기 중(WAITLISTED) 상태에서는 결제 불가
- 결제 완료 시 결제 대기에서 수강 확정 상태로 전환

**취소 정책**
- 결제 전 상태(PENDING, WAITLISTED)는 언제든 취소 가능
  - 수강 신청 후 강의 시작 3일 전까지 결제하지 않으면 자동 취소 처리
- 결제 완료 후에는 아래 두 조건을 모두 만족하는 경우에만 취소 가능
  - 결제 완료 후 7일 이내
  - 강의 시작 3일 전까지
- 위 조건 중 하나라도 만족하지 않으면 취소 불가
  - 예) 결제 후 7일이 지났다면 강의 시작 전이라도 취소 불가
  - 예) 결제 후 7일 이내라도 강의 시작 3일 전이라면 취소 불가
- 취소 시 대기 중인 첫 번째 신청자가 자동으로 결제 대기 상태로 전환
- 대기 중(WAITLISTED) 취소 시 정원에 영향을 주지 않음
...
단, 강의 시작 3일 전이 지난 경우에는 자리가 생겨도 `OPEN` 으로 복귀하지 않음.
...
목록 정렬 기준은 강의 상태(OPEN → DRAFT → CLOSED) 우선, 동일 상태 내에서는 시작일 오름차순으로 정렬.
...
대기열 기능 구현을 위해 `WAITLISTED` 상태를 추가.
...
이미 결제되었거나 취소된 건, 대기 중(WAITLISTED) 상태인 건은 결제 불가.
...
- `CONFIRMED` 상태는 결제 완료 후 7일 이내 AND 강의 시작 3일 전까지 두 조건 모두 만족 시 취소 가능
- 취소 발생 시 강의가 `OPEN` 상태인 경우에만 대기열의 첫 번째 신청자를 자동으로 `PENDING` 으로 승격
- `WAITLISTED` 상태 취소는 정원을 차지하지 않으므로 정원 복구 없이 상태만 변경

결제 완료 시점 기준을 `updatedAt` 으로 사용.
`confirm()` 호출 시 JPA Auditing 이 `updatedAt` 을 갱신하며, 현재 `Enrollment.updatedAt` 을 변경하는 지점은 결제 확정과 수강 취소뿐이므로 기준이 변질될 가능성이 없다고 판단.
...
정원 초과 시 신청을 거부하는 대신 대기열(`WAITLISTED`)에 자동 등록하도록 구현.

동시성 문제는 비관적 락(`SELECT ... FOR UPDATE`)으로 처리.
수강 신청과 수강 취소 시 `Course` 에 락을 걸고 조회하여 동시에 여러 요청이 들어와도 순차적으로 처리되도록 함.
...
Spring Security 는 구현 범위에서 제외. 현재는 요청 파라미터로 `userId` 를 직접 받아 사용자를 식별.
...
> 현재 수강 인원을 별도 집계 쿼리 없이 `Course.enrollmentCnt` 필드를 직접 증감하는 방식으로 관리

집계 쿼리(`COUNT`) 방식 대신 필드 직접 관리 방식을 선택한 이유는 아래와 같음.
...
> 대기열을 별도 테이블로 분리하지 않고 `Enrollment` 의 상태값(`WAITLISTED`)으로 관리
...
> 결제 완료 시점을 별도 컬럼으로 관리하지 않고 JPA Auditing 의 `updatedAt` 을 활용

결제 확정(`confirm()`) 호출 시 JPA Auditing 이 `updatedAt` 을 자동으로 갱신.
현재 `Enrollment` 의 `updatedAt` 을 변경하는 지점은 결제 확정과 수강 취소뿐이므로 기준이 변질될 가능성이 없다고 판단.
...
서비스 레이어에서 엔티티의 상태를 직접 조작하지 않고, 엔티티 내부에 도메인 메서드를 두어 비즈니스 규칙을 응집.
...
단, 예외 처리는 엔티티에 두지 않고 서비스 레이어에서 담당하도록 분리.
엔티티가 애플리케이션 계층의 예외 클래스에 의존하면 순수한 도메인 객체로서의 역할이 훼손되기 때문.

##
### 6. QueryDSL — 동적 쿼리 처리

강의 목록 조회는 강사명, 제목, 가격 범위, 기간, 상태 등 다양한 조건을 선택적으로 조합할 수 있어야 함.
`@Query` JPQL 로는 조건마다 분기 처리가 필요해 유지보수가 어려워지므로 QueryDSL 을 도입.

QueryDSL 은 조건이 `null` 이면 해당 조건을 자동으로 제외하여, 조건이 추가되어도 메서드 하나만 추가하면 되므로 확장성이 높음.
중복 강의 검증도 파라미터 8개의 JPQL 대신 QueryDSL 의 `selectOne + fetchFirst` 방식으로 전환하여 `LIMIT 1` 쿼리로 불필요한 집계 연산을 제거함.
...
이를 방지하기 위해 조회 쿼리에 `JOIN FETCH` 를 적용하여 한 번의 쿼리로 필요한 데이터를 모두 가져오도록 처리.

##
### 8. 자동 처리 — Spring Batch 도입

미결제 자동 취소, 대기열 만료 처리, 강의 자동 마감을 스케줄러 대신 Spring Batch 로 구현.

단순 `@Scheduled` 방식도 가능하지만, 수강생이 늘어날 경우를 고려해 아래와 같은 이유로 Spring Batch 를 선택.

- 청크 단위(100건) 처리로 대용량 데이터에서도 메모리를 안전하게 유지할 수 있음
- skip / retry 정책으로 일부 건 처리 실패 시에도 나머지 건은 계속 처리 가능
- JobRepository 를 통해 실행 이력이 자동으로 기록되어 모니터링이 용이함
...
**인증/인가**
Spring Security 를 구현 범위에서 제외. 로그인 및 인증/인가 기능이 없어 현재는 요청 파라미터로 `userId` 를 직접 받아 사용자를 식별하고 있음. Security 도입 후 인증 토큰 기반으로 변경 예정.

### 제약사항

현재 로그인 기능이 없어 요청 파라미터로 `userId` 를 직접 전달하는 방식으로 사용자를 식별.
이로 인해 타인의 `userId` 를 입력하면 해당 사용자의 수강 신청 내역에 접근할 수 있는 보안 취약점이 존재함.
...
| 2 | 학생11의 수강 신청 목록 확인 (enrollmentId WAITLISTED 확인) | `GET /api/v1/enrollment/14` |
| 3 | 학생11이 WAITLISTED 상태 결제 시도 → 실패 | `PATCH /api/v1/enrollment/14/{enrollmentId}/pay` |
| 4 | 학생7이 enrollmentId 7 취소 | `DELETE /api/v1/enrollment/10/7` |
| 5 | 취소된 enrollmentId 7 결제 시도 → 실패 | `PATCH /api/v1/enrollment/10/7/pay` |
...
본 프로젝트는 Claude 를 활용하여 개발하였으며, 활용 범위는 아래와 같음.
